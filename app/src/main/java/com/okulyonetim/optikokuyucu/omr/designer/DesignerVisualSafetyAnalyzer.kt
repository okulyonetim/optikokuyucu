 ...
 *
 * Bubble reading samples both the filled centre and a local background ring, so visual content is
 * kept outside a guard radius larger than the bubble itself. Box interiors stay usable: only the
 * actual four strokes are treated as ink.
 */
object DesignerVisualSafetyAnalyzer {
        ...
        document.visualElements.forEach { element ->
            ...
            if (!bounds.isInside(template.space)) {
                issues += ReadabilityIssue(
                    ...
                    type = ReadabilityIssueType.VISUAL_EDGE_CLEARANCE,
                    elementIds = listOf("visual:${element.id}"),
                    message = "Görsel öğe form sınırlarının dışına taşıyor."
                )
            }
            ...
            marks.forEach { mark ->
                ...
                if (visualTouchesCircle(element, mark.center, guardRadius)) {
                    issues += ReadabilityIssue(
                        severity = ReadabilitySeverity.ERROR,
                        type = ReadabilityIssueType.VISUAL_MARK_CLEARANCE,
                        elementIds = listOf("visual:${element.id}", mark.id),
                        message = "Görsel öğe OMR işaretinin güvenli okuma alanına giriyor."
                    )
                }
            }

            template.fiducials.forEach { fiducial ->
                val protectedMarker = expand(fiducial.bounds, fiducialClearance)
                if (visualTouchesRect(element, protectedMarker)) {
                    issues += ReadabilityIssue(
                        severity = ReadabilitySeverity.ERROR,
                        type = ReadabilityIssueType.VISUAL_FIDUCIAL_CLEARANCE,
                        elementIds = listOf("visual:${element.id}", "marker:${fiducial.markerId}"),
                        message = "Görsel öğe marker güvenli alanına giriyor."
                    )
                }
            }
        }
    ...
    private fun visualTouchesCircle(
        ...
        is DesignerLineElement ->
            distancePointToSegment(center, element.start, element.end) <= radius + element.strokeWidth / 2.0
    ...
    private fun visualTouchesRect(
        element: DesignerVisualElement,
        rect: TemplateRect
    ): Boolean = when (element) {
        is DesignerTextElement -> rectsOverlap(element.bounds, rect)
        is DesignerLineElement -> segmentIntersectsRect(
            element.start,
            element.end,
            expand(rect, element.strokeWidth / 2.0)
        )
    ...
    private fun boxEdges(element: DesignerBoxElement): List<Pair<TemplatePoint, TemplatePoint>> {
        val left = element.bounds.left
        val top = element.bounds.top
        val right = element.bounds.right
        val bottom = element.bounds.bottom
        val topLeft = TemplatePoint(left, top)
        val topRight = TemplatePoint(right, top)
        val bottomRight = TemplatePoint(right, bottom)
        val bottomLeft = TemplatePoint(left, bottom)
        return listOf(
            topLeft to topRight,
    ...
    // CanonicalBubbleReader samples local background out to 1.75× radius.
