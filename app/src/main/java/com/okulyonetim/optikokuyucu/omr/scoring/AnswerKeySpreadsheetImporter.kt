...
/** Parsed spreadsheet answer key plus an optional booklet value carried by the file. */
...
/**
 * Offline spreadsheet importer. XLSX is parsed directly from OOXML and legacy XLS is read through
 * JExcelAPI. The exported AnswerKeyXlsxExporter layout is fully reversible, while a simple
 * two-column "Soru / Doğru Cevap" sheet is also accepted.
 */
object AnswerKeySpreadsheetImporter {
    ...
    internal fun parseRows(
        ...
        rows.forEach { row ->
            val key = row.getOrNull(0).orEmpty().trim().lowercase(Locale("tr", "TR"))
        ...
        val headerIndex = rows.indexOfFirst { row ->
            val a = row.getOrNull(0).orEmpty().trim().lowercase(Locale("tr", "TR"))
            val b = row.getOrNull(1).orEmpty().trim().lowercase(Locale("tr", "TR"))
            (a == "soru" || a == "soru no" || a == "soru numarası") &&
                ("cevap" in b || "doğru" in b)
        }
        ...
        require(missing.isEmpty()) {
            "Cevap anahtarı eksik. ${missing.size} soru için cevap bulunamadı."
        }
