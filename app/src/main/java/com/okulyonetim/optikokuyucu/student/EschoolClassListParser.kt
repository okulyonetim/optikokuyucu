...
object EschoolClassListParser {
    ...
    fun parse(
        ...
        text.lineSequence().forEach { rawLine ->
            ...
            val row = studentRow.matchEntire(line) ?: return@forEach
            val number = StudentNumber.normalize(row.groupValues[2])
            val name = row.groupValues[3].trim().replace(Regex("\\s+"), " ")
            if (number.isBlank() || name.isBlank()) return@forEach
            students += StudentRosterEntry(
                studentNumber = number,
                fullName = name,
                gender = StudentGender.fromEschool(row.groupValues[4]),
