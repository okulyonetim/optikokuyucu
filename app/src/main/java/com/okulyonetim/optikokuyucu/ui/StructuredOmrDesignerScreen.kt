...
fun StructuredOmrDesignerScreen(openCvReady: Boolean, onBack: () -> Unit, onOpenAdvanced: () -> Unit) {
    ...
    fun clearEditing() {
        ...
        bookletDraft = null; bookletPatternText = ""
        descriptionDraft = null
        imageDraft = null; imageEditorOpen = false
    }
    ...
    fun saveDocument() {
        ...
        status = runCatching {
            ...
            document = stored; formName = stored.name; "Kaydedildi · v${stored.version}"
    ...
    fun storeComponent(component: com.okulyonetim.optikokuyucu.omr.designer.DesignerOmrComponent) {
        ...
        document = if (existing == null) document.copy(components = document.components + component)
        else DesignerDocumentEditor.replaceComponent(document, component)
    ...
    numberDraft?.let { draft ->
        NumberAreaEditorScreen(
            ...
            { numberDraft = it },
            { text -> numberPatternText = text; DesignerAreaCatalog.parseNumberPattern(text)?.let { numberDraft = numberDraft?.copy(values = it) } },
            { clearEditing() },
            { storeComponent(it) }
        ); return
    }
    answerDraft?.let { draft ->
        AnswerAreaEditorScreen(
            ...
            { answerDraft = it },
            { text -> answerPatternText = text; DesignerAreaCatalog.parseAnswerPattern(text)?.let { answerDraft = answerDraft?.copy(choices = it) } },
            { clearEditing() },
            { storeComponent(it) }
        ); return
    }
    bookletDraft?.let { draft ->
        BookletAreaEditorScreen(
            document, draft, bookletPatternText,
            { bookletDraft = it },
            { text -> bookletPatternText = text; DesignerAreaCatalog.parseBookletPattern(text)?.let { bookletDraft = bookletDraft?.copy(choices = it) } },
            { clearEditing() },
            { storeComponent(it) }
        ); return
    }
    descriptionDraft?.let { draft ->
        DescriptionAreaEditorScreen(document, draft, { descriptionDraft = it }, { clearEditing() }, { storeVisual(it) }); return
    }
    if (imageEditorOpen) {
        ImageAreaEditorScreen(document, imageDraft, { imageDraft = it }, { clearEditing() }, { storeVisual(it) }); return
    }
...
private fun EditorTopBar(onBack: () -> Unit, onSave: () -> Unit) {
    Surface(modifier = Modifier.fillMaxWidth(), color = MaterialTheme.colorScheme.primary) {
        Row(modifier = Modifier.fillMaxWidth().padding(2.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text("Yeni Optik Form", modifier = Modifier.weight(1f), color = MaterialTheme.colorScheme.onPrimary, fontWeight = FontWeight.Medium)
...
private fun FormInformationCard(
    ...
    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp)) {
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            ...
            OutlinedTextField(value = formName, onValueChange = onFormNameChange, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Ad *") }, singleLine = true, shape = RoundedCornerShape(50.dp))
...
private fun <T> DropdownField(label: String, value: String, options: List<T>, optionLabel: (T) -> String, onSelected: (T) -> Unit, modifier: Modifier = Modifier) {
    ...
    Box(modifier = modifier) {
        Column(verticalArrangement = Arrangement.spacedBy(3.dp)) {
            ...
            Surface(modifier = Modifier.fillMaxWidth().clickable { expanded = true }, shape = RoundedCornerShape(50.dp), border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant)) {
                Row(modifier = Modifier.padding(horizontal = 14.dp, vertical = 11.dp), verticalAlignment = Alignment.CenterVertically) { Text(value, modifier = Modifier.weight(1f), maxLines = 1); Text("⌄") }
            }
        }
...
private fun OpticalFormAreaHeader(onAdd: () -> Unit) {
    Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 4.dp), verticalAlignment = Alignment.CenterVertically) {
        ...
        Surface(modifier = Modifier.size(28.dp).clickable(onClick = onAdd), shape = CircleShape, color = MaterialTheme.colorScheme.secondaryContainer) { Box(contentAlignment = Alignment.Center) { Text("+", fontWeight = FontWeight.Bold) } }
    }
}
